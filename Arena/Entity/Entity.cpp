...
void Entity::constructBody()
{
    ...
    _groundHitSensor.setOnHitCallback([&](float speed){
        hit(SensorListener::GROUND_HIT_SENSOR_TYPE, speed);
    });
...
void Entity::stepLeft()
{
    ...
    if (_leftSensor.isActive())
        return;
    if (_body->GetLinearVelocity().x >= -12.0f)
